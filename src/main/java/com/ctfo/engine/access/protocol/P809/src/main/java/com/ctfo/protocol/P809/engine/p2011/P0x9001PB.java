...
/**
 * 从链路连接请求消息，从链路，DOWN_CONNECT_REQ
 * <br>
 * 消息方向：上级平台往下级平台
 *
 * @author liangyi
 ...
 */
public class P0x9001PB implements IPassBody {
    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x9001消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x9001 p = (P0x9001) model;
            return ConvUtil.unSigned32LongToBigBytes(p.getVerifyCode());
        } catch (Exception e) {
            throw new P809Exception("P0x9001编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 4 > data.length) {
            throw new P809Exception("P0x9001消息体解码失败，消息为空或者长度不足。");
        }
        try {
            ...
            throw new P809Exception("P0x9001解码失败:", e);
        }
    }
}
