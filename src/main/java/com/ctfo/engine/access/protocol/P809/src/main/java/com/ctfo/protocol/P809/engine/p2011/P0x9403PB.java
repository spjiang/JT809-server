...
import com.ctfo.protocol.P809.model.p2011.P0x9403;
 ...
 * @date 2021/2/23 9:56
 */
public class P0x9403PB implements IPassBody {
    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x9403消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x9403 p = (P0x9403) model;
            ...
            throw new P809Exception("P0x9403编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 43 > data.length) {
            throw new P809Exception("P0x9403消息体解码失败，消息为空或者长度不足。");
        }
        try {
            P0x9403 p = new P0x9403();
            int pos = 0;
            ...
            throw new P809Exception("P0x9403解码失败:", e);
        }
    }
}
