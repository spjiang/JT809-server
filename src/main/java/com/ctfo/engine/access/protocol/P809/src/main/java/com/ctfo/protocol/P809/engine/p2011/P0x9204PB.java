...
import com.ctfo.protocol.P809.model.p2011.P0x9204;
 ...
 */
public class P0x9204PB implements IPassBody {
    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x9204消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x9204 p = (P0x9204) model;
            ...
            throw new P809Exception("P0x9204编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 28 > data.length) {
            throw new P809Exception("P0x9204消息体解码失败，消息为空或者长度不足。");
        }
        try {
            P0x9204 p = new P0x9204();
            int pos = 0;
            ...
            System.arraycopy(data, pos, carInfo, 0, p.getDataLength());
            p.setCarInfo(ConvUtil.bytes2String(carInfo));
            return p;
        } catch (Exception e) {
            throw new P809Exception("P0x9204解码失败:", e);
        }
    }
}
