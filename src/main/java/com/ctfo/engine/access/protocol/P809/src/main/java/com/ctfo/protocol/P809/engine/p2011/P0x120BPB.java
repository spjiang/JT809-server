...
import com.ctfo.protocol.P809.model.p2011.P0x120B;
 ...
 */
public class P0x120BPB implements IPassBody {

    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x120B消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x120B p = (P0x120B) model;
            ...
            throw new P809Exception("P0x120B编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 32 > data.length) {
            throw new P809Exception("P0x120B消息体解码失败，消息为空或者长度不足。");
        }
        try {
            P0x120B p = new P0x120B();
            int pos = 0;
            ...
            throw new P809Exception("P0x120B解码失败:", e);
        }
    }
}
