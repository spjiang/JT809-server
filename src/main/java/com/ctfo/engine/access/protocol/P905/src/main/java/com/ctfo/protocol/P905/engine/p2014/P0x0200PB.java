...
/**
 * 位置信息汇报消息编解码处理类<code>P0x0200</code>
 * <p>
 * 消息ID：0x0200
 *
 * @author 凉意 2018年4月22日 下午2:50:33
 * @see
 * @version 1.0
 */
public class P0x0200PB implements IPassBody {

	public byte[] bodyToBytes(PModel model) throws P905Exception {
		if (null == model) {
			throw new P905Exception("P0x0200编码失败,传入消息对象为空。");
		}
		try {
			P0x0200 p = (P0x0200) model;
			...
			throw new P905Exception("P0x0200编码失败:", e);
		}
	}

	public PModel bodyFromBytes(byte[] data) throws P905Exception {
		if (null == data || 25 > data.length) { // 消息不足25字节不做解析，抛出异常
			throw new P905Exception("P0x0200解析失败，消息长度不足25字节。");
		}
		try {
			...
			throw new P905Exception("P0x0200解码失败:", e);
		}
