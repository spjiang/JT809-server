...
import com.ctfo.protocol.P809.model.p2011.P0x9209;
 ...
 */
public class P0x9209PB implements IPassBody {
    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x9209消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x9209 p = (P0x9209) model;
            ...
            throw new P809Exception("P0x9209编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 29 > data.length) {
            throw new P809Exception("P0x9209消息体解码失败，消息为空或者长度不足。");
        }
        try {
            P0x9209 p = new P0x9209();
            int pos = 0;
            ...
            throw new P809Exception("P0x9209解码失败:", e);
        }
    }
}
