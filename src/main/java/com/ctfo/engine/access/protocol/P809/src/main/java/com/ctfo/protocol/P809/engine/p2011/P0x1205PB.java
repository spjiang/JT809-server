...
import com.ctfo.protocol.P809.model.p2011.P0x1205;
...
/**
 * 启动车辆定位信息交换应答消息，主链路，UP_EXG_MSG_RETURN_STARTUP_ACK
 * <br>
 * 本条消息是下级平台对上级平台下发的 DOWN_EXG_MSG_RETURN_STARTUP 消息的应答消息
 *
 * @author liangyi
 * @date 2021/1/12 9:09
 */
public class P0x1205PB implements IPassBody {

    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x1205消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x1205 p = (P0x1205) model;
            ...
            throw new P809Exception("P0x1205编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 28 > data.length) {
            throw new P809Exception("P0x1205消息体解码失败，消息为空或者长度不足。");
        }
        try {
            P0x1205 p = new P0x1205();
            int pos = 0;
            ...
            throw new P809Exception("P0x1205解码失败:", e);
        }
    }
}
