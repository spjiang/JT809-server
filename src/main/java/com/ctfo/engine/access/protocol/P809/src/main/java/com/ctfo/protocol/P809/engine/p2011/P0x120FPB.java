...
import com.ctfo.protocol.P809.model.p2011.P0x120F;
 ...
 */
public class P0x120FPB implements IPassBody {

    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x120F消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x120F p = (P0x120F) model;
            ...
            throw new P809Exception("P0x120F编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 34 > data.length) {
            throw new P809Exception("P0x120F消息体解码失败，消息为空或者长度不足。");
        }
        try {
            P0x120F p = new P0x120F();
            int pos = 0;
            ...
            throw new P809Exception("P0x120F解码失败:", e);
        }
    }
}
