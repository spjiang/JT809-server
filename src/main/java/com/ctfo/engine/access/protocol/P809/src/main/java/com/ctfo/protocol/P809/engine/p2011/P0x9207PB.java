...
import com.ctfo.protocol.P809.model.p2011.P0x9207;
...
public class P0x9207PB implements IPassBody {
    ...
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x9207消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            ...
            throw new P809Exception("P0x9207编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 29 > data.length) {
            throw new P809Exception("P0x9207消息体解码失败，消息为空或者长度不足。");
        }
        try {
            ...
            throw new P809Exception("P0x9207解码失败:", e);
        }
    }
}
