...
import com.ctfo.protocol.P809.model.p2011.P0x1505;
 ...
 */
public class P0x1505PB implements IPassBody {
    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x1505消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x1505 p = (P0x1505) model;
            ...
            throw new P809Exception("P0x1505编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 29 > data.length) {
            throw new P809Exception("P0x1505消息体解码失败，消息为空或者长度不足。");
        }
        try {
            P0x1505 p = new P0x1505();
            int pos = 0;
            ...
            throw new P809Exception("P0x1505解码失败:", e);
        }
    }
}
