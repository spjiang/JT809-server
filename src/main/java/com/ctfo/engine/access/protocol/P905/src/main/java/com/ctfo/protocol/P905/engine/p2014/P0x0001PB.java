...
import com.ctfo.protocol.P905.model.p2014.P0x0001;
...
public class P0x0001PB implements IPassBody {
	...
	public byte[] bodyToBytes(PModel model) throws P905Exception {
		if (null == model) {
			throw new P905Exception("P0x0001消息体编码失败，传入消息体实体对象为空。");
		}
		try {
	...
	public PModel bodyFromBytes(byte[] data) throws P905Exception {
		if (null == data || 5 != data.length) {
			throw new P905Exception("P0x0001消息体解码失败，消息长度必须为5字节。");
		}
		try {
			...
			throw new P905Exception("P0x0001解码失败:", e);
		}
	}
