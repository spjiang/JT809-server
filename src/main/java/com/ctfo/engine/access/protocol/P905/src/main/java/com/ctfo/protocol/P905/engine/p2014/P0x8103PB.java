...
import com.ctfo.protocol.P905.model.p2014.P0x8103;
...
/**
 * 终端参数设置消息编解码处理类<code>P0x8103</code>
 *
 * <p>
 * 消息ID：0x8103
 *
 * @author 凉意 2018年4月26日 上午10:12:42
 * @see
 * @version 1.0
 */
public class P0x8103PB implements IPassBody {

	public byte[] bodyToBytes(PModel model) throws P905Exception {
		if (null == model) {
			throw new P905Exception("P0x8103消息体编码失败，传入消息体实体对象为空。");
		}
		try {
			P0x8103 p = (P0x8103) model;
			List<P0x8103VO> dataList = p.getParams();
			IoBuffer buf = IoBuffer.allocate(1).setAutoExpand(true);
			for (P0x8103VO pvo : dataList) {
				...
				if (idHex.equals("0001")) {// UINT32 终端心跳发送间隔，秒（默认30秒）
					...
					buf.putInt(Integer.parseInt(value));
				} else if (idHex.equals("001A")) { // STRING 一卡通主服务器地址,IP或域名（保留）
					buf.put(ConvUtil.string2Bytes(value));
				} else if (idHex.equals("001B")) { // UINT32 一卡通主服务器TCP端口（保留）
					buf.putInt(Integer.parseInt(value));
				} else if (idHex.equals("001C")) { // STRING	一卡通备份服务器地址IP或域名（保留）
					buf.put(ConvUtil.string2Bytes(value));
				} else if (idHex.equals("001D")) { // UINT32	一卡通备份服务器TCP端口（保留）
					buf.putInt(Integer.parseInt(value));
					...
					buf.putInt(Integer.parseInt(value));
				} else if (idHex.equals("0049")) { // String 监听电话号码
					buf.put(ConvUtil.string2Bytes(value));
			...
			throw new P905Exception("P0x8103编码失败:", e);
		}
	}

	public PModel bodyFromBytes(byte[] data) throws P905Exception {
		if (null == data || 0 >= data.length) {
			throw new P905Exception("P0x8103解析失败，消息为空或者长度不足。");
		}
		int dataLen = data.length;
		int pos = 0;
		P0x8103 p = new P0x8103();
		...
		try {
			while (pos < dataLen) {
				...
				if (idHex.equals("0001")) {// UINT32 终端心跳发送间隔，秒（默认30秒）
					...
					pvo.setParamVal(ConvUtil.bytes2String(value));
				} else if (idHex.equals("00B3")) { //UINT32	视频服务器无线通信拨号密码
					pvo.setParamVal(ConvUtil.bytes2String(value));
				} else if (idHex.equals("00B4")) { //STRING	视频服务器地址,IP或域名
					pvo.setParamVal(ConvUtil.bytes2String(value));
				} else if (idHex.equals("00B5")) { // UINT32	视频服务器端口
			...
			throw new P905Exception("P0x8103解码失败:", e);
		}
