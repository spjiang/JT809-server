...
import com.ctfo.protocol.P809.model.p2011.P0x1503;
 ...
 */
public class P0x1503PB implements IPassBody {

    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x1503消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x1503 p = (P0x1503) model;
            ...
            throw new P809Exception("P0x1503编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 33 > data.length) {
            throw new P809Exception("P0x1503消息体解码失败，消息为空或者长度不足。");
        }
        try {
            P0x1503 p = new P0x1503();
            int pos = 0;
            ...
            pos += 4;
            p.setMsgId(ConvUtil.bigBytes2Unsigned32Long(data, pos));
            pos += 4;
            p.setResult(data[pos]);
            return p;
        } catch (Exception e) {
            throw new P809Exception("P0x1503解码失败:", e);
        }
    }
}
