...
import com.ctfo.protocol.P809.model.p2011.P0x120C;
 ...
 */
public class P0x120CPB implements IPassBody {
    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x120C消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x120C p = (P0x120C) model;
            ...
            throw new P809Exception("P0x120C编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 64 > data.length) {
            throw new P809Exception("P0x120C消息体解码失败，消息为空或者长度不足。");
        }
        try {
            P0x120C p = new P0x120C();
            int pos = 0;
            ...
            throw new P809Exception("P0x120C解码失败:", e);
        }
    }
}
