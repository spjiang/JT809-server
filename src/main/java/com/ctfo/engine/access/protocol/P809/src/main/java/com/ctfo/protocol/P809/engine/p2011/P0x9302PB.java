...
import com.ctfo.protocol.P809.model.p2011.P0x9302;
 ...
 * @date 2021/2/23 9:04
 */
public class P0x9302PB implements IPassBody {
    @Override
    public byte[] bodyToBytes(PModel model) throws P809Exception {
        if (null == model) {
            throw new P809Exception("P0x9302消息体编码失败，传入消息体实体对象为空。");
        }
        try {
            P0x9302 p = (P0x9302) model;
            ...
            throw new P809Exception("P0x9302编码失败:", e);
        }
    }
    ...
    public PModel bodyFromBytes(byte[] data) throws P809Exception {
        if (null == data || 27 > data.length) {
            throw new P809Exception("P0x9302消息体解码失败，消息为空或者长度不足。");
        }
        try {
            P0x9302 p = new P0x9302();
            int pos = 0;
            ...
            throw new P809Exception("P0x9302解码失败:", e);
        }
    }
}
